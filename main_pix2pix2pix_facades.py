...
parser.add_argument('--netG2', default='',
            help="path to netG2 (to continue training)")
...
def main():
    ...
    netG2 = pix2pix._netG(args.verbose)
    netG2.apply(pix2pix.weights_init)
    if args.netG2 != '':
        netG2.load_state_dict(torch.load(args.netG2))
    print(netG2)
    ...
    optimizerG = optim.Adam(netG.parameters(), lr=args.lr,
                            betas=(args.beta1, 0.999))
    optimizerG2 = optim.Adam(netG2.parameters(), lr=args.lr,
                            betas=(args.beta1, 0.999))

    for epoch in range(args.start_epoch, args.epochs):

        # Train for one epoch
        train(dataloaders['train'], netG, netG2, netD, criterion, criterion_L1,
            optimizerG, optimizerG2, optimizerD, epoch, input, noise, fixed_noise,
            label, nz)
...
def train(train_loader, netG, netG2, netD, criterion, criterion_L1, optimizerG,
        optimizerG2, optimizerD, epoch, input, noise, fixed_noise, label, nz):
    '''
    ...
    for i, (img, gt) in enumerate(train_loader):
        ...
        if args.useL1:
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f Loss_L1: %.4f Loss_L1G2: %.4f D(x): %.4f D(G(z)): %.4f / %.4f'
                  % (epoch, args.epochs, i, len(train_loader),
                     errD.data[0], errG.data[0], errL1.data[0], errL1G2.data[0], D_x, D_G_z1, D_G_z2))
        else:
